...
export function TimelinePreviousButton() {
    ...
    return (        <div className="flex items-center">
            <button
                ...
                disabled={isDisabled}
                className={cn(
                    "size-8 md:size-10 rounded-full bg-white/80 backdrop-blur-sm border border-border shadow-md hover:bg-white/90 transition-all flex items-center justify-center text-black",
                    isDisabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer hover:border-red-300 hover:shadow-lg'
                )}
            >
                <ChevronLeft animateOnHover size={24} className={cn(isDisabled ? 'opacity-50' : '')} />
            </button>
        </div>
    )
}

export function TimelineControls() {    const { isPlaying, setIsPlaying } = useStepper()
    const { mode } = useNavigationMode()
    ...
    return (        <div className="flex items-center justify-center">
            <button
                ...
                className="size-9 md:size-12 rounded-full bg-white/80 backdrop-blur-sm border border-border shadow-md hover:bg-white/90 hover:shadow-lg transition-all flex items-center justify-center cursor-pointer"
            >
                {isPlaying
                    ? <Pause animateOnHover size={20} fill='black' />
                    : <Play animateOnHover size={20} fill='black' />
                }
            </button>
        </div>
    )
...
export function TimelineNextButton() {
    ...
    return (        <div className="flex items-center">
            ...
            >
                <ChevronRight animateOnHover size={24} className={cn(isDisabled ? 'opacity-50' : '')} />
            </button>
        </div>
    )
}